...
def parse_args():
    ...
    parser.add_argument(
        "--score-thresh", default=0.4, type=float, help="samples to visualize"
    )
    parser.add_argument(
...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        # ====== [新增] 将 GT / Pred 的向量坐标落盘为 JSON ======

        def _rescale_pts_to_vis(pts):
            # 这里不做缩放，仅确保是 float，保持与你的 BEV 坐标一致（pc_range 空间）
            return [[float(x), float(y)] for x, y in pts]

        # 1) 取 GT 折线（优先用 polyline 的原始点；若你没传 polyline，可回退 fixed_num）
        gt_items = []
        if "polyline_pts" in args.gt_format:
            gt_lines_instance = gt_bboxes_3d[0].instance_list
            for line, cls in zip(gt_lines_instance, gt_labels_3d[0]):
                pts = np.array(list(line.coords), dtype=np.float32)
                gt_items.append({"cls": int(cls), "polyline": _rescale_pts_to_vis(pts)})
        else:
            gt_lines_fixed = gt_bboxes_3d[0].fixed_num_sampled_points
            for pts, cls in zip(gt_lines_fixed, gt_labels_3d[0]):
                pts = pts.numpy().astype(np.float32)
                gt_items.append({"cls": int(cls), "polyline": _rescale_pts_to_vis(pts)})

        # 2) 取 Pred 折线（按 score 阈值 keep 掉）
        pred_items = []
        keep = scores_3d > args.score_thresh
        for s, lab, pts in zip(scores_3d[keep], labels_3d[keep], pts_3d[keep]):
            pts = pts.numpy().astype(np.float32)
            pred_items.append(
                {
                    "cls": int(lab),
                    "score": float(s),
                    "polyline": _rescale_pts_to_vis(pts),
                }
            )

        # 3) 组织并落盘
        vec_payload = {
            "sample_token": pts_filename,
            "pc_range": [
                float(pc_range[0]),
                float(pc_range[1]),
                float(pc_range[3]),
        ...
        with open(osp.join(sample_dir, "vectors.json"), "w") as f:
